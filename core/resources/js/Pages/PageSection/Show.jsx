...
const Show = ({ section }) => {
    ...
    const rawMappingConfig = parseJsonSafely(section.mapping_config);
    ...
    const mappingGroups = normalizeMappingConfig(rawMappingConfig);
    ...
    return (
        <>
                        ...
                        <div className="card mb-4">
                            <div className="card-header d-flex justify-content-between align-items-center">
                                ...
                                <span className="badge bg-warning text-dark">
                                    {mappingGroups.reduce((sum, g) => sum + (g.fields?.length || 0), 0)}
                                </span>
                            </div>
                            <div className="card-body p-0">
                                {mappingGroups.length > 0 ? (
                                    <>
                                            ...
                                            <div className="d-flex align-items-start">
                                                ...
                                                <div>
                                                    ...
                                                    <div className="small text-muted mt-2">
                                                        Use group markers: <code>{'<!-- START REPEATABLE group_name -->'}</code> ... <code>{'<!-- END REPEATABLE group_name -->'}</code>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="list-group list-group-flush">
                                            {mappingGroups.map((g) => (
                                                <div key={g.group_name} className="list-group-item border-bottom">
                                                    <div className="d-flex justify-content-between align-items-center mb-2">
                                                        <strong>{g.group_label} <small className="text-muted">({g.group_name})</small></strong>
                                                        <span className="badge bg-warning text-dark">{g.fields?.length || 0}</span>
                                                    </div>
                                                    {(g.fields || []).map((field, index) => (
            ...
            {section.mapping_enabled && section.html_template && mappingGroups.length > 0 && (
                    ...
                    <div className="card-body">
                        ...
                        <div className="border rounded p-4 bg-white">
                            ...
                            <div className="row g-3">
                                {mappingGroups.flatMap((g) => (g.fields || []).map((field, index) => (
                                    <div key={`${g.group_name}_${index}`} className="col-md-6">
                                        <div className="border rounded p-3 bg-light">
                                            <div className="d-flex align-items-center mb-2">
                                                <i className={`bx ${getFieldIcon(field.type)} text-warning me-2`}></i>
                                                <div>
                                                    <div className="text-muted small">{field.label} <small className="text-muted">({g.group_name})</small></div>

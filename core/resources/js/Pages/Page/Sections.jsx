...
const buildSectionDataJsonPayload = (section) => ({
    ...
    mapping_items:
        ...
        typeof section.mapping_items === "object" &&
        !Array.isArray(section.mapping_items)
            ? section.mapping_items
            : {},
...
const SectionBuilder = () => {
        ...
        if (activeSections && activeSections.length > 0) {
                ...
                if (activeSection.pivot?.section_data) {
                    try {
                        ...
                        if (parsed) {
                            ...
                            mappingItemsByGroup = normalizeMappingItemsByGroup(
                                parsed.mapping_items,
                            );
                        }
                    } catch (e) {
                        parsedData = {};
            ...
            setTimeout(() => {
                ...
                formatted.forEach((section, index) => {
                    ...
                    Object.entries(section.mapping_items || {}).forEach(
                        ([groupName, items]) => {
                                ...
                                Object.entries(it || {}).forEach(
                                    ([fieldName, value]) => {
                                        if (
                                            ...
                                            allPreviews[
                                                `${index}_mapping_${groupName}_${itemIndex}_${fieldName}`
                                            ] = value;
                                        }
                                        if (
                                            Array.isArray(value) &&
                                            ...
                                            value.forEach((nested, ni) => {
                                                Object.entries(
                                                    ...
                                                    if (
                                                        ...
                                                        allPreviews[
                                                            `${index}_mapping_${groupName}_${itemIndex}_${fieldName}_${ni}_${nf}`
                                                        ] = nv;
                                                    }
                                                });
                                            });
    ...
    const getChildMappingGroups = (groups, parentGroupName) =>
        (groups || []).filter(
            (g) => g.parent_group && g.parent_group === parentGroupName,
        );

    const isTopLevelMappingGroup = (g) =>
        !g.parent_group || String(g.parent_group).trim() === "";
    ...
    const createSectionFingerprint = (section) =>
        JSON.stringify({
    ...
    const buildSectionDefaults = (selectedSection) => {
        ...
        const groups = normalizeMappingConfig(
            selectedSection.mapping_config || [],
        );
        groups.forEach((group) => {
            const makeDefaultItem = (defaults = {}) => {
                const item = {};
                (group.fields || []).forEach((fc) => {
                    if (Object.prototype.hasOwnProperty.call(defaults, fc.name)) {
                        item[fc.name] = defaults[fc.name];
            ...
            const defaultItems = Array.isArray(group.default_items) && group.default_items.length
                ? group.default_items.map((item) => makeDefaultItem(item))
                : [makeDefaultItem()];
            if (!mapping_items[group.group_name]) {
                mapping_items[group.group_name] = [];
            }
            mapping_items[group.group_name].push(...defaultItems);
        });
    ...
    const removeSectionFromState = (index) => {
        ...
        Object.keys(filePreviews).forEach((k) => {
            if (k.startsWith(`${index}_`)) delete newPreviews[k];
        });
                ...
                return {
                    ...
                    mapping_items: {
                        ...
                        [parentGroup]: parentItems.map((row, j) => {
                            ...
                            const nestedItems = Array.isArray(row[nestedGroup])
                                ? row[nestedGroup]
                                : [];
                            return {
                                ...row,
    ...
    const addMappingItem = (si, groupName) => {
        ...
        const group =
            groups.find((g) => g.group_name === groupName) || groups[0];
        if (!group) return;
        ...
        if (!ns[si].mapping_items[groupName])
            ns[si].mapping_items[groupName] = [];
        ns[si].mapping_items[groupName].push(newItem);
        ns[si].dirty = true;
        setSections(ns);
    };

    const addNestedMappingItem = (
        si,
        parentGroup,
        parentIndex,
        nestedGroupName,
    ...
    const removeNestedMappingItem = (
        ...
        const ns = [...sections];
        if (!ns[si]?.mapping_items?.[parentGroup]?.[parentIndex]?.[nestedGroup])
            return;
        ns[si].mapping_items[parentGroup][parentIndex][nestedGroup].splice(
            nestedIndex,
            1,
        );
        if (ns[si].mapping_files?.[parentGroup]?.[parentIndex]?.[nestedGroup]) {
            delete ns[si].mapping_files[parentGroup][parentIndex][nestedGroup][
                nestedIndex
            ];
        }
    ...
    const removeMappingItem = (si, groupName, ii) => {
        ...
        setFilePreviews((prev) => {
            ...
            Object.keys(np).forEach((k) => {
                const pkPrefix = `${si}_mapping_${groupName}_`;
                if (k.startsWith(pkPrefix)) {
                    const afterPrefix = k.substring(pkPrefix.length);
                    const itemIdxStr = afterPrefix.split("_")[0];
                    const itemIdx = parseInt(itemIdxStr);

                    if (itemIdx === ii) {
                        if (np[k]?.startsWith("blob:"))
                            URL.revokeObjectURL(np[k]);
                        delete np[k];
                    } else if (itemIdx > ii) {
                        keysToUpdate.push({
                            oldKey: k,
            ...
            keysToUpdate.forEach(({ oldKey, oldIdx, newIdx }) => {
                const newKey = oldKey.replace(
                    `_mapping_${groupName}_${oldIdx}_`,
                    `_mapping_${groupName}_${newIdx}_`,
                );
                np[newKey] = np[oldKey];
                delete np[oldKey];
            });
        ...
        if (ns[si].mapping_files?.[groupName]) {
            const oldFiles = ns[si].mapping_files[groupName];
            const newFiles = {};
            let newIdx = 0;
            ...
            Object.keys(oldFiles)
                ...
                .forEach((oldIdx) => {
                    if (oldIdx !== ii) {
                        newFiles[newIdx] = oldFiles[oldIdx];
                        newIdx++;
                    }
                });

            ns[si].mapping_files[groupName] = newFiles;
        }
    ...
    const handleFileUpload = (
        ...
        if (
            isMapping &&
            mapCtx?.groupName != null &&
            mapCtx.itemIndex != null
        ) {
            const { groupName, itemIndex, nestedGroup, nestedItemIndex } =
                mapCtx;
            if (!ns[si].mapping_files) ns[si].mapping_files = {};
            if (!ns[si].mapping_files[groupName])
                ns[si].mapping_files[groupName] = {};
            if (!ns[si].mapping_files[groupName][itemIndex])
                ns[si].mapping_files[groupName][itemIndex] = {};
            if (nestedGroup != null && nestedItemIndex != null) {
                if (!ns[si].mapping_files[groupName][itemIndex][nestedGroup])
                    ns[si].mapping_files[groupName][itemIndex][nestedGroup] =
                        {};
                if (
                    !ns[si].mapping_files[groupName][itemIndex][nestedGroup][
                        nestedItemIndex
                ...
                if (!ns[si].mapping_items[groupName])
                    ns[si].mapping_items[groupName] = [];
                if (!ns[si].mapping_items[groupName][itemIndex])
                    ns[si].mapping_items[groupName][itemIndex] = {};
        ...
        if (file.type.startsWith("image/")) {
            ...
            const pk =
                ...
                mapCtx?.groupName != null &&
                mapCtx.itemIndex != null
                    ? mapCtx.nestedGroup != null &&
                      mapCtx.nestedItemIndex != null
    ...
    const removeFile = (si, fn, isMapping = false, mapCtx = null) => {
        ...
        if (
            ...
            if (nestedGroup != null && nestedItemIndex != null) {
                if (
                    ...
                    delete ns[si].mapping_files[groupName][itemIndex][
                        nestedGroup
                    ][nestedItemIndex][fn];
                }
                if (
                ...
                if (ns[si].mapping_files?.[groupName]?.[itemIndex])
                    delete ns[si].mapping_files[groupName][itemIndex][fn];
                if (ns[si].mapping_items?.[groupName]?.[itemIndex])
                    ns[si].mapping_items[groupName][itemIndex][fn] = "";
            }
        } else {
        ...
        const pk =
            isMapping && mapCtx?.groupName != null && mapCtx.itemIndex != null
    ...
    const getSelectedSection = (id) =>
        availableSections.find((s) => s.id == id);
    ...
    const renderFieldInput = (
        ...
        const nestedItemIndex = isMapping
            ? (mapCtx?.nestedItemIndex ?? null)
            : null;
        const pk = isMapping
            ...
            : `${si}_${fn}`;
        const nestedFileLeaf =
            isMapping && nestedGroup != null && nestedItemIndex != null
                ? sections[si]?.mapping_files?.[groupName]?.[itemIndex]?.[
                      nestedGroup
                  ]?.[nestedItemIndex]?.[fn]
                : null;
        ...
        const fileMapCtx =
            isMapping && groupName != null && itemIndex != null
        ...
        switch (field.type) {
            case "link": {
                ...
                const updateLink = (key, nextValue) =>
                    onChange({ ...linkValue, [key]: nextValue });

                return (
                    <div className="sb-link-fields">
                        <input
                            type="text"
                            className="sb-input"
            ...
            case "image":
                return (
                    <div>
                        ...
                        <label
                            className="sb-file-input-label"
                            htmlFor={`file-${si}-${fn}${isMapping ? `-m-${groupName}-${itemIndex}${nestedGroup != null ? `-n-${nestedGroup}-${nestedItemIndex}` : ""}` : ""}`}
                        >
                            <Upload size={14} />
                            <span>
                        ...
                        <p className="sb-file-hint">
                            JPG, PNG, GIF, SVG, WebP — max 3 MB
                        </p>
                    </div>
                );
            case "file":
                return (
                    <div>
                        ...
                        <p className="sb-file-hint">
                            Images, video, PDF, Word — max 3 MB
                        </p>
                    </div>
                );
            default:
    ...
    const handleSubmit = (e) => {
        ...
        const changedSections = sections.filter((section) => {
            ...
            if (section.files && Object.keys(section.files).length > 0)
                return true;
            if (
                section.mapping_files &&
                Object.keys(section.mapping_files).length > 0
            )
                return true;
            if (!section.originalFingerprint) return true;
            return (
                section.originalFingerprint !==
                createSectionFingerprint(section)
            );
        });
            ...
            if (section.mapping_files) {
                const appendMappingFileLeaves = (branch, pathParts) => {
                    ...
                    Object.entries(branch).forEach(([k, val]) => {
                        if (isFileObject(val)) {
                            const tail = [...pathParts, k]
                                .map((p) => `[${p}]`)
                                .join("");
                            formData.append(
                                `sections[${index}][mapping_files]${tail}`,
                                val,
                            );
                        } else if (Array.isArray(val)) {
                            val.forEach((entry, i) => {
                                if (entry && typeof entry === "object")
                                    appendMappingFileLeaves(entry, [
                                        ...pathParts,
                                        k,
                                        String(i),
                                    ]);
        ...
        router.post(getSectionStoreRoute(), formData, {
            ...
            onError: (errors) => {
                setIsSaving(false);
                toast.error(
                    "Error saving: " + (errors.message || "Unknown error"),
                );
            },
        });
    };
    ...
    return (
      ...
      `}</style>
            ...
            <DeleteConfirmationModal
                ...
                itemName={
                    deleteTarget !== null
                        ? getSelectedSection(sections[deleteTarget]?.section_id)
                              ?.name
                        : null
                }
            ...
            <div className="sb-wrap">
                ...
                <div className="sb-page-header">
                    ...
                    <button
                        className="sb-drag-toggle"
                        onClick={() => setIsDraggable(!isDraggable)}
                    >
                        <GripVertical size={14} />
                        Drag & Drop
                        <span
                            className={`sb-toggle-pill ${isDraggable ? "on" : ""}`}
                        />
                    </button>
                </div>
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    {sections.map((section, sIndex) => {
                        ...
                        const hasMapping =
                            selectedSection?.mapping_enabled &&
                            selectedSection?.mapping_config;
                        ...
                        return (
                            <div
                                ...
                                onDragStart={(e) =>
                                    handleSectionDragStart(e, sIndex)
                                }
                            ...
                            >
                                ...
                                >
                                    <div className="sb-section-header-left">
                                        ...
                                        <div className="sb-section-icon">
                                            <LayoutTemplate size={14} />
                                        </div>
                                        <span className="sb-section-name">
                                            {selectedSection?.name ||
                                                "Unknown Section"}
                                        </span>
                                        <div className="sb-badges">
                                            <span className="sb-badge sb-badge-num">
                                                #{sIndex + 1}
                                            </span>
                                            {hasMapping && (
                                                <span className="sb-badge sb-badge-repeat">
                                                    <Repeat
                                                        size={9}
                                                        style={{
                                                            verticalAlign:
                                            ...
                                            {section.pivot_id && (
                                                <span className="sb-badge sb-badge-saved">
                                                    Saved
                                                </span>
                                            )}
                                        </div>
                                            ...
                                            {fields.length > 0 ? (
                                                fields.map((field, fIndex) => {
                                                    ...
                                                    return (
                                                        <div
                                                            key={fIndex}
                                                            className={`sb-field-group ${isFullWidth ? "sb-field-full" : ""}`}
                                                        >
                                                                                                ...
                                                                                                {(
                                                                                                    ...
                                                                                                    (
                                                                                                        ...
                                                                                                        <div
                                                                                                            ...
                                                                                                            className={`sb-field-group ${field.type === "code" ? "sb-field-full" : ""}`}
                                                                                                        >
                                                                                                            <label className="sb-label">
                                                                                                                {fieldTypeIcons[
                                                                                                                    field
                                                                                                ...
                                                                                                (
                                                                                                    ...
                                                                                                    return (
                                                                                                        <div
                                                                                                            ...
                                                                                                            className="sb-mapping-nested"
                                                                                                            style={{
                                                                                                                marginTop: 12,
                                                                                                                paddingLeft: 12,
                                                                                                                borderLeft:
                                                                                                                    "2px solid var(--sb-border, #e5e7eb)",
                                                                                                            }}
                                                                                                        >
                                                                                                            <div
                                                                                                                className="sb-mapping-header"
                                                                                                                style={{
                                                                                                                    marginBottom: 8,
                                                                                                                }}
                                                                                                            >
                                ...
                                {!section.isCollapsed && !selectedSection && (
                                    <div className="sb-section-body">
                                        <div className="sb-warn">
                                            Section template not found. Please
                            ...
                            <div className="sb-picker-body">
                                {filteredSections.length === 0 ? (
                                    <div className="sb-picker-empty">
                                        <div className="sb-picker-empty-icon">
                                            <Blocks size={36} />
                                        </div>
                                        <p
                                            style={{
                                        ...
                                        {searchTerm && (
                                            <button
                                                type="button"
                                                className="sb-btn-outline sb-btn-sm"
                                                onClick={() =>
                                                    setSearchTerm("")
                                                }
                                            >
                                    ...
                                    <>
                                        <p className="sb-picker-prompt">
                                            {sections.length === 0
                                                ? "Pick your first section to get started"
                                            ...
                                            {paginatedSections.map(
                                                (section) => (
                                                    ...
                                                    >
                                                        ...
                                                        <div className="sb-comp-id">
                                                            {section.identifier}
                                                        </div>
                                                        <span
                                                            className={`sb-comp-tag ${section.mapping_enabled ? "sb-comp-tag-repeat" : "sb-comp-tag-regular"}`}
                                                        >
                                        ...
                                        {totalPages > 1 && (
                                            <>
                                                    ...
                                                    {Array.from(
                                                        ...
                                                        (_, i) => {
                                                            ...
                                                            return (
                                                                <button
                                                                    key={pn}
                                                                    className={`sb-page-btn ${currentPage === pn ? "active" : ""}`}
                                                                    onClick={() =>
                                                                        setCurrentPage(
                                                                            pn,
                                                                        )
                                                ...
                                                <p className="sb-pagination-meta">
                                                    Page {currentPage} of{" "}
                                                    {totalPages} ·{" "}
                                                    {startIndex + 1}–
                                                    {Math.min(
                    ...
                    {sections.length > 0 && (
                        <div className="sb-save-bar">
                            ...
                            >
                                {isSaving ? (
                                    <>
                                        <div className="sb-spinner" /> Saving…
                                    </>
                                ) : (
                                    <>
